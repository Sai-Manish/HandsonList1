...
Date: 25-02-2020
Description: This program will perform write lock - record locking.
*/
...
int main()
{
    ...
    lock.l_type = F_WRLCK;
    lock.l_start = sizeof(ticket) * 1; //write locking second record
    ...
    if (ret != -1)
    {
        ticket *ticket1 = (ticket *)malloc(sizeof(ticket));
         lseek(fd, sizeof(ticket)*1, SEEK_CUR);
